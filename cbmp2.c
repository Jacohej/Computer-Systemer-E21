...
int erosion(unsigned char input_image[BMP_WIDTH][BMP_HEIGTH], unsigned char output_image[BMP_WIDTH][BMP_HEIGTH]){
    int check = 0; int found = 0; int area[7][7] = {0}; int weight[7][7] = {0}; int sum = 0;
    check = 0;
    ...
    for (int i = 0; i < 7; i++){
        for (int j = 0; j < 7; j++)
        {
            if (i == 0 || i == 6 || j == 0 || j == 6) weight[i][j] = 1;
            if ((i >= 1 && i <= 5) && (j >= 1 && j <= 5)) weight[i][j] = 4;
            if ((i >= 2 && i <= 4) && (j >= 2 && j <= 4)) weight[i][j] = 16;
            if (i == 3 && j == 3) weight[i][j] = 0;
            //printf("weight x: %d y: %d value: %d \n", i, j, weight[i][j]);
        }
            ...
            if ( input_image[x][y] > 120){
                ...
                for (int xx = x - 3; xx <= x + 3; xx++){
                    for (int yy = y - 3; yy <= y + 3; yy++){
                        if (xx < 0 || xx > BMP_WIDTH - 1|| yy < 0 || yy > BMP_HEIGTH - 1){
                            area[xx - x + 3][yy - y + 3] = 0;
                        } else {
                            area[xx - x + 3][yy - y + 3] = (input_image[xx][yy] / 255) * weight[xx - x + 3][yy - y + 3];
                        }
                        sum += area[xx - x + 3][yy - y + 3];
                        //printf("Area x: %d y: %d value: %d \n", xx - x + 3, yy - y + 3, area[xx - x + 3][yy - y + 3]);
                    }
                }
                //printf("sum: %d \n", sum);
                if (sum < 190){
